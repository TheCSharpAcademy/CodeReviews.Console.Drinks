...
{
    private readonly ApiService _apiService = new ApiService("https://www.thecocktaildb.com/api/json/v1/1");
    public async Task<string?> SelectCategoryAsync()
    {
        var categoryList = await _apiService.GetAsync<Dictionary<string, List<Category>>>("list.php?c=list");
    ...
    public async Task<string?> SelectDrinkAsync(string? category)
    {
        var drinkList = await _apiService.GetAsync<Dictionary<string, List<Drink>>>($"filter.php?c={category}");
    ...
    {
        var drink = await _apiService.GetAsync<Dictionary<string, List<Drink>>>($"lookup.php?i={drinkId}");
